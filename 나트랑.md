...
- **위치 검색(강화됨):** Nominatim(나트랑 viewbox 우선) + **Photon**(POI/상호 검색 강함) 두 소스 동시 조회 → 근접순 정렬 → Google Maps iframe 미리보기 → 2단계 확정. `runSearch`/`_searchNominatim`/`_searchPhoton`
- **지도에서 직접 핀 찍기:** 모달 안 Leaflet 미니맵(`#mPickMap`), 탭/드래그로 위치 지정 → `confirmPin()`이 `_mState.loc`에 저장. 추가·편집 공용. 상호 검색 안 될 때의 대안. 핀 위치도 길찾기에 그대로 사용됨
